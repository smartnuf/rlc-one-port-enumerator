function gen_unique_two_terminal_hamiltonian_graphs ( N1, N2, M1, M2, show_figures )
    ...
    % Only graphs for which every non-terminal node lies on a Hamiltonian path between the
    % two terminal nodes (i.e. there exists a Hamiltonian path from node 1 to node 2) are collected.
    %
        ...
        for M = M1 : M2
            ...
            if ( M <= num_edges )
                ...
                for i = 1 : size ( combinations, 1 )
                    ...
                    if ( all ( conncomp ( G ) == 1 ) )
                        % Check if there exists a Hamiltonian path from node 1 to node 2.
                        % This ensures that every non-terminal node lies on a path between the terminals.
                        if ( ~ has_hamiltonian_path_from_1_to_2 ( G ) )
                            continue;
                        end
    ...
    fprintf ( 'Number of unique two-terminal Hamiltonian graphs with %d to %d nodes and %d to %d edges:\n', N1, N2, M1, M2 );
    fprintf ( 'N\\E\t' );
    for M = M1 : M2
        fprintf ( '%d\t', M );
    end
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    % Nested helper function to check for a Hamiltonian path from node 1 to node 2
    function flag = has_hamiltonian_path_from_1_to_2 ( G )
        N_nodes = numnodes ( G );
        A = adjacency ( G );
        visited = false ( 1, N_nodes );
        visited ( 1 ) = true;
        flag = dfs ( 1, 1, visited );

        function found = dfs ( current, count, visited )
            if ( current == 2 && count == N_nodes )
                found = true;
                return;
            end
            found = false;
